...
import com.example.demoapp1.recyclerview.EmployeeModelData;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create Table TABLE_NAME(employeeId   TEXT primary key, employeeName TEXT)");
    }
    ...
    public Boolean updateEmployeeData(String employeeId,String employeeName) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("employeeName", employeeName);
        @SuppressLint("Recycle") Cursor cursor = db.rawQuery("select * from TABLE_NAME where employeeId = ?", new String[]{employeeId});
        if (cursor.getCount() >= 0) {
            long result = db.update(TABLE_NAME, contentValues, "employeeId=?", new String[]{employeeId});
            return result != -1;
        }return false;
    }
